...
{  
	...
	/*if(cansend==false && message.data[0]!='\0')
	{
			 ...
			 memcpy(buffer[ctr].data,message.data,20);
			 ctr++;
			 datahere=true;
	}*/
	if(message.data[0]!='\0')
		     ...
		     memcpy(buffer[ctr].data,message.data,20);
			   ctr++;
		        ...
		        packet.acknum=ackonum;                 
             	memcpy(packet.payload,buffer[head].data,20);	
                i= computeChecksum(packet);
		        printf("check retur %d\n",i);
		        packet.checksum=i;
		        printf("check returned %d\n", packet.checksum);  
    ...
    int calc=0,i;
		...
		printf("calc value is%d  pktchecksum %d\n",calc, packet.checksum);
          ...
          if(packet.acknum==seqnum && calc==packet.checksum)   //handle corruption here
		  {
						 ...
						 { 
							...
							for(i=0;i<ctr;i++)
							{
                                ...
                                printf("sending buffer data");	
                                 ...
                                 packet.acknum=ackonum;
                                 memcpy(packet.payload,temp.data,20);
                                 packet.checksum=computeChecksum(packet);								 
								tolayer3(0,packet); 
                               // snum = packet.seqnum;  //1
                               if(ctr==0)
			  ...
			  printf("Packet discarded,wrong ACK");
	...
	{  
		 ...
		 printf("q");*/
		  ackonum=!ackonum;
                 ...
                 { 
			       printf("error in packet received,out of order");
                   //packet.acknum=!ackonum;
		           //tolayer3(1,packet);
                 }  
    }
