...
start_process (void *exec_)
{
  ...
  hash_init(&(thread_current()->map_head), page_hash, page_less, NULL);
...
bool lazy_load_page(struct file* file, off_t ofs, uint8_t* upage, uint32_t readbytes, uint32_t zerobytes, bool writable)
{
	...
	if(file_read(file, kpage, readbytes) != (int)readbytes)
	{
		palloc_free_page(kpage);
		return false;
	}
	memset(kpage + readbytes, 0, zerobytes);


	//if(!pagedir_set_page(thread_current()->pagedir, upage, kpage, false))
	if(!install_page(upage, kpage, writable))
	{
		palloc_free_page(kpage);
		return false;
...
{
	uint32_t read_bytes = readbytes;
	uint32_t zero_bytes = zerobytes;
	bool first = true;

	//printf("Lazy load segment\n");

	uint8_t* u_page = upage;
	off_t of_s = ofs;

	while(readbytes > 0 || zerobytes > 0)
	{
		size_t page_read_bytes = readbytes < PGSIZE ? readbytes: PGSIZE;
		size_t page_zero_bytes = PGSIZE - page_read_bytes;

		struct cs_map * temp = malloc(sizeof(struct cs_map));
		temp->uvaddr = upage;
		temp->file = file;
		temp->ofs = ofs;
		temp->readbytes = readbytes;
		temp->zerobytes = zerobytes;
		temp->loaded = false;
		temp->writable = writable;
		//temp->next = NULL;
		hash_insert(&(thread_current()->map_head), &(temp->elem));

		if(first)
		{
			first = false;
			//printf("laoding firest\n");
			lazy_load_page(file, ofs, upage, readbytes, zerobytes, writable);
			//printf("First done!!\n");
		}

		/*if(thread_current()->map_head == NULL)
		{
			thread_current()->map_head = temp;
		}
		else
		{
			struct cs_map* it = thread_current()->map_head;
			while(it->next != NULL)
			{
				it = it->next;
			}
	...
	//load_segment(file, of_s, u_page, read_bytes, zero_bytes, writable);
