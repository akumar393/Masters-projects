...
{
const struct cs_map *p = hash_entry (elem, struct cs_map, elem);
...
UNUSED)
{
const struct cs_map *c = hash_entry (a, struct cs_map, elem);
const struct cs_map *d = hash_entry (b, struct cs_map, elem);
return c->uvaddr < d->uvaddr;
}
