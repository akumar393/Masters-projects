...
#define AUTHOR_STATEMENT "I, akumar39, have read and understood the course academic integrity policy."
...
void init_response(int sock_index, char* cntrl_payload)
{
		...
		uint16_t payload_len, response_len;
		payload_len = sizeof(void);// Discount the NULL chararcter

    char *cntrl_response_header, *cntrl_response_payload, *cntrl_response;
	cntrl_response_payload = (char *) malloc(payload_len);
	memcpy(cntrl_response_payload,"\0",sizeof("\0"));
