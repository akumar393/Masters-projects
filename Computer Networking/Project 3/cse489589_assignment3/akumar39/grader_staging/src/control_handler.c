...
//#include "../include/control_payload_lib.h"
...
bool control_recv_hook(int sock_index)
{
    ...
    if(payload_len != 0){
        ...
        if(recvALL(sock_index, cntrl_payload, payload_len) < 0){
            ...
            return FALSE;
  			...
  			uint64_t napa;
                 memcpy(&napa,cntrl_payload,sizeof(cntrl_payload));
                 printf("payload is %d",ntohs(napa));
    ...
    switch(control_code){
		...
		//	           BUILD_BUG_ON(sizeof(struct CONTROL_HEADER) != payload_len); // This will FAIL during compilation itself; See comment above.
