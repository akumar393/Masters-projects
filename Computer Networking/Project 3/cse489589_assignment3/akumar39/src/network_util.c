...
{
    ssize_t bytes = 0;
    bytes = recvfrom(sock_index, buffer, nbytes, 0,(struct sockaddr*)&router_addr,addrlen);

    if(bytes == 0) return -1;
    while(bytes != nbytes)
        bytes += recvfrom(sock_index, buffer+bytes, nbytes-bytes, 0,(struct sockaddr*)&router_addr,addrlen);
...
{
    ssize_t bytes = 0;
    bytes = sendto(sock_index, buffer, nbytes, 0,(struct sockaddr*)&router_addr,addrlen);

    if(bytes == 0) return -1;
    while(bytes != nbytes)
        bytes += sendto(sock_index, buffer+bytes, nbytes-bytes, 0,(struct sockaddr*)&router_addr,addrlen);
