        ...
        for(sock_index=0; sock_index<=head_fd; sock_index+=1){
            ...
            if(FD_ISSET(sock_index, &watch_list)){
                ...
                /* router_socket */
					...
					printf("\nhello there ,welcome to routing updates port\n");
					timestamp =receive_routing_updates(sock_index,tv.tv_sec);
                       	
	            while(ticks<3*timestamp)
		       {	 
		             timestamp=receive_routing_updates(sock_index,timestamp);
			         ticks++;
		        }	
		 
		        if(ticks==3*timestamp)
			              setcosttoInfinity(timestamp);								
